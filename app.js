...
function respondInWorkspace(message) {
  if (!chatStream) return;
  ...
  const response = workspaceOffline
    ? `Offline cache engaged. ${depthHint} response queued with cached rituals. I will mark where live citations normally appear.`
    : `Online mode. ${depthHint} response with fresh web context. I can also pull a 30-year council for "${message}" if you want.`;
  const agentMessage = { role: 'agent', meta, text: response, overlay: overlayLabel };
...
function updateWorkspaceStatus() {
  ...
  if (composerStatus) {
    composerStatus.textContent = workspaceOffline ? 'Offline · using cached playbooks' : 'Online · live web pulls available';
  }
